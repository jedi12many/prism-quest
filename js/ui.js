...
const GUIDE_PAGES = [
  ...
  { icon: '💎', title: 'The shattered Prismblade', html: `The legendary <b>Prismblade</b> lies broken — one facet hidden in each land as a <b>sparkle in the grass</b> that <b>glows brighter as you get near it</b>. Each facet is a fine weapon alone; the village <b>Glassworks</b> fuses 2–4 into something far greater. Find all four for total devastation.` },
...
function facetHint(s) {
  ...
  return `<br><br>✨ Old tale: the <b>Prismblade</b> shattered into four facets, one lost in each land. Watch for a <b>sparkle</b> in the grass — it <b>glows brighter the closer you wander to it</b>, so sweep a land corner to corner. The ${missing.map(z => ZONES[z].dir).join(', ')} still hide${missing.length === 1 ? 's' : ''} theirs. The <b>Glassworks</b> kiln can fuse any two or more.`;
}
